...
class AssignmentEvaluator:
    def __init__(
        ...
        # State model is still used for skill diagnostics and KPI estimation.
    ...
    def evaluate(self, assignment: dict[str, str]) -> CandidateScore:
        ...
        objective_evaluation = self.objective_evaluations
        ...
        total = (
            ...
            + weights["makespan"] * schedule_score
    ...
    def strategy_weights(self) -> dict[str, float]:
        """Return normalized objective weights for KPI, makespan, and cost."""
        base_mapping = {
            "KPI_FOCUSED": {"kpi": 0.60, "makespan": 0.25, "cost": 0.15},
            "COST_FOCUSED": {"kpi": 0.25, "makespan": 0.25, "cost": 0.50},
            "SPEED_FOCUSED": {"kpi": 0.25, "makespan": 0.55, "cost": 0.20},
            "BALANCED": {"kpi": 0.45, "makespan": 0.35, "cost": 0.20},
        }
        if self.metric_weights:
            return self.metric_weights
        return dict(base_mapping.get(self.strategy, base_mapping["BALANCED"]))

    def _normalize_metric_weights(self, weights: dict[str, float] | None) -> dict[str, float] | None:
        if not isinstance(weights, dict):
            return None
        allowed = ("kpi", "makespan", "cost")
    ...
    def resource_skill_score(self, task_id: str, resource_id: str) -> float:
        """Return the skill fit score between a resource and a task.

        Prefer skillMatch from RESOURCE_TASK_CAPABILITY edges in the state model.
        Fall back to raw task/resource skill data when no edge is available.
        """
        # Prefer pre-computed capability edges from the state model graph.
    ...
    def resource_assignment_heuristic_score(
        ...
        construction, pitch-adjustment, and candidate ordering aligned with the
        KPI/makespan/cost metric weights configured by the user.
        """
        weights = self.strategy_weights()
        resource = self.resource_by_id.get(resource_id, {})
        task = self.task_by_id.get(task_id, {})
        ...
        makespan_fit = (capacity_fit + slack_fit) / 2.0
        ...
        kpi = min(100.0, sum(safe_float(impact.get("weight")) for impact in task.get("kpiImpacts") or []) * 100.0)

        return (
            weights["kpi"] * kpi
            + weights["makespan"] * makespan_fit
            + weights["cost"] * cost
        )
    ...
    def _build_capability_index(self) -> dict[tuple[str, str], dict[str, Any]]:
        """Parse RESOURCE_TASK_CAPABILITY edges for O(1) lookup."""
        ...
        # Build node lookup: node_id to resource/task attributes.
    ...
    def state_model_info(self) -> dict[str, Any]:
        """Return debug information about the loaded state model."""
        return {

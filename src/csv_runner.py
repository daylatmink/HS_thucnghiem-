...
def runtime_diagnostics(result: Any) -> dict[str, Any]:
    duration = max(0.0, (result.finished_at - result.started_at).total_seconds())
    iterations = max(0, int(result.iterations_run or 0))
    return {
        "durationSeconds": round(duration, 6),
        "iterationsRun": iterations,
        "secondsPerIteration": round(duration / iterations, 6) if iterations else None,
    }
...
def run_from_csv(args: argparse.Namespace) -> dict[str, Any]:
        ...
        options=coerce_hs_options(
            {
                ...
                "maxEvaluations": getattr(args, "max_evaluations", None),
                "fixedBudget": getattr(args, "fixed_budget", False),
                "populationSize": args.population_size,
                "crossoverRate": args.crossover_rate,
                "mutationRate": args.mutation_rate,
                "elitismCount": args.elitism_count,
                "tournamentSize": args.tournament_size,
                "hybridHsRatio": args.hybrid_hs_ratio,
            }
        ),
    ).run()
    ...
    output = {
        ...
        "objectiveEvaluations": evaluator.objective_evaluations,
        "evaluationAtBest": result.best.score.diagnostics.get("objectiveEvaluation"),
        "converged": result.converged,
        "startedAt": result.started_at.isoformat(),
        "finishedAt": result.finished_at.isoformat(),
        "runtime": runtime_diagnostics(result),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--max-evaluations", type=int, default=None, help="Maximum objective evaluations/NFE budget.")
    parser.add_argument(
        "--fixed-budget",
        action="store_true",
        help="Record convergence but continue stochastic optimizers until max evaluations is exhausted.",
    )
